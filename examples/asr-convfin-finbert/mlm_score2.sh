...
# train_step_checkpoints="1000 10000 20000 50000"
...
bert_lm="$bert_model"
score_output="scored/${am}${ngram_lm}${lstm_lm}/${n}best_${decode_set}_finbert_${cased}.combined.ac${ac_scale}.json"
...
if [ $stage -le 2 ]; then
    ...
    rescore_output="scored/rescored_${nbest_name}_bert-base-finnish-${cased}-v1.combined.ac${ac_scale}.${weight}.json"
    mlm rescore \
        --model "$bert_lm" \
        --weight $weight \
        "${nbest_dir}/ac_combined.ac_scale${ac_scale}.json" \
